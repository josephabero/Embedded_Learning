...
void FlashDevice::init()
{
    ...
    DS();
    SPI.Initialize(Spi::SpiPort::kPort2, 8, Spi::FrameModes::kSPI, 48);
}
...
void FlashDevice::WriteByte(uint32_t address, uint8_t data)
{
    ...
    {
        sjsu::LogWarning("Write operation FAILED. "
                        "Write Enable Command must be issued before writing.");
    }
}
...
void FlashDevice::BlockErase(uint8_t address)
{
    ...
    {
        sjsu::LogWarning("Block Erase operation FAILED. "
                        "Write Enable Command must be issued beforehand.");
    }
}
